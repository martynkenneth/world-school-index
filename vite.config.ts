...
export default defineConfig(async () => {
  ...
  const hostingConfig = JSON.parse(await readFile(hostingConfigPath, "utf8")) as {
    d1: string | null;
    r2: string | null;
  };
  const sitesPluginUrl = pathToFileURL(
    resolve(process.cwd(), "build", "sites-vite-plugin.ts"),
  ).href;
  const { sites } = await import(sitesPluginUrl);
